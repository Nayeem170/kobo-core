...
pub fn truncate_to_width(s: &str, px: f32, max_w: usize) -> String {
    ...
    let ellipsis = "...";
    let ellipsis_w = measure_text(ellipsis, px);
    let budget = max_w.saturating_sub(ellipsis_w);
    if budget == 0 {
        return s.chars().take(3).chain(ellipsis.chars()).collect();
    }
    ...
    for word in s.split(' ') {
        if !first {
            if current_w + space_w > budget as f32 {
                break;
            }
            out.push(' ');
            current_w += space_w;
        ...
        let word_w = if word.is_empty() {
            ...
            text_render::word_width(word, px)
        };
        if current_w + word_w > budget as f32 {
            break;
        }
        out.push_str(word);
        current_w += word_w;
    ...
    if out.is_empty() {
        for ch in s.chars() {
            ...
            if current_w + ch_w > budget as f32 {
                break;
            }
            out.push(ch);
            current_w += ch_w;
