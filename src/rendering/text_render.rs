...
/// The faces compiled into this crate, for consumers that need to hand font
/// bytes to another engine. `rendering::svg` registers them with resvg so the
/// text inside a diagram uses the same typefaces as the prose around it.
pub fn embedded_font_data() -> [&'static [u8]; 2] {
    [FONT_LATIN, FONT_MONO]
}
...
impl Script {
    ...
    /// CJK scripts: heavy fonts (4-9 MB), lazy-loaded, never present as stock
    /// system fonts. Used to skip the system-font fallback scan.
    pub fn is_cjk(self) -> bool {
        matches!(self, Script::Japanese | Script::Chinese | Script::Korean)
    }
...
pub fn decode_image(raw: &[u8], max_w: usize, max_h: usize) -> Option<DecodedImage> {
    // SVG first: `image` cannot decode it, and a figure that falls through to
    // `None` here renders as its caption with no picture.
    if super::svg::is_svg(raw) {
        return super::svg::rasterize_svg(raw, max_w, max_h);
    }
